...
WireToolSetup.open( "uv_wiremanager", "Waypoint Manager", "gmod_wire_uvmanager", nil, "Waypoint Managers" )

---@class TOOL.UVWire : Structure.TOOL, TOOL
local TOOL = TOOL
local uvws = UnitVehiclesWaypointsSystem
if CLIENT then
    language.Add( "Tool.wire_uv_wiremanager.name", uvws.Texts.WireSupport.ToolName)
    language.Add( "Tool.wire_uv_wiremanager.desc", uvws.Texts.WireSupport.ToolDesc)
end
...
function TOOL.BuildCPanel(panel)
    WireDermaExts.ModelSelect(panel, "wire_uv_wiremanager_model", list.Get( "[UV] WireManager Model List" ), 4, true)
end

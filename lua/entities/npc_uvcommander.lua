...
if SERVER then	
	...
	function ENT:OnRemove()
		...
		if !self.wrecked and self.v then
	...
	function ENT:Wreck()
		if IsValid(self.v) and !self.v.wrecked then
			...
			if IsValid(self.e) and isfunction(self.e.GetDriver) and IsValid(UVGetDriver(self.e)) and UVGetDriver(self.e):IsPlayer() then 
				--UVGetDriver(self.e):PrintMessage( HUD_PRINTCENTER, "Commander "..v.." ☠ Combo Bounty x"..uvcombobounty..": "..bounty)
				UVNotifyCenter({UVGetDriver(self.e)}, "uv.hud.combo", "UNITS_DISABLED", "uv.unit.commander", v, bountyplus, uvcombobounty)
			end
			uvwrecks = uvwrecks + 1
						...
						if dist:LengthSqr() > fedist:LengthSqr() then
							if fvectdot > 0 then
								if uvcalm and fdist:LengthSqr() < 100000 then
									throttle = 0
								elseif fdist:LengthSqr() < 100000 and self.e:GetVelocity():LengthSqr() > 200000 then
									if self.v:GetVelocity():LengthSqr() > f.v:GetVelocity():LengthSqr() and fdist:Dot(forward) > 0 and !self.formationpoint then
										throttle = 2
									end
								end
							end
	...
	function ENT:Initialize()
		...
		if UVUOneCommander:GetBool() then
			self.voice = 12
		else
		...
		if !uvtargeting then self.v:EmitSound( "npc/strider/strider_skewer1.wav" ) end

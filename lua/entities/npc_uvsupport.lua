...
if SERVER then	
	...
	function ENT:Wreck()
		if IsValid(self.v) and !self.v.wrecked then
			...
			if IsValid(self.e) and isfunction(self.e.GetDriver) and IsValid(UVGetDriver(self.e)) and UVGetDriver(self.e):IsPlayer() then 
				//UVGetDriver(self.e):PrintMessage( HUD_PRINTCENTER, "Support "..v.." ☠ Combo Bounty x"..uvcombobounty..": "..bounty)
				UVNotifyCenter({UVGetDriver(self.e)}, "uv.hud.combo", "UNITS_DISABLED", "uv.unit.support", v, bountyplus, uvcombobounty)
			end
			uvwrecks = uvwrecks + 1
	...
	function ENT:Initialize()
		...
		self:SetHealth(-1)
		self.bountytimer = CurTime()
		self.callsign = "Support "..self:EntIndex()
		self.moving = CurTime()
		...
		if !uvtargeting then self.v:EmitSound( "buttons/lever4.wav" ) end

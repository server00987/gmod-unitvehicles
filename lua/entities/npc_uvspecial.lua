...
if SERVER then	
	...
	function ENT:OnRemove()
		...
		if self.metwithenemy and !uvresourcepointsrefreshing and uvresourcepoints > 1 and !self.v.rhino and !uvonecommanderactive then
	...
	function ENT:Wreck()
		if IsValid(self.v) and !self.v.wrecked then
			...
			if IsValid(self.e) and isfunction(self.e.GetDriver) and IsValid(UVGetDriver(self.e)) and UVGetDriver(self.e):IsPlayer() then
				if self.v.rhino then
					UVGetDriver(self.e):PrintMessage( HUD_PRINTCENTER, "Rhino "..v.." ☠ Combo Bounty x"..uvcombobounty..": "..bounty)
				else
					UVGetDriver(self.e):PrintMessage( HUD_PRINTCENTER, "Special "..v.." ☠ Combo Bounty x"..uvcombobounty..": "..bounty)
				end
			end
			uvwrecks = uvwrecks + 1
			...
			elseif self.v:GetClass() == "prop_vehicle_jeep" then
				...
				wreck:EmitSound( "physics/metal/metal_large_debris2.wav" )
	...
	function ENT:Patrol()
		...
		if self.v.rhino and !uvtargeting then
			SafeRemoveEntity(self)
		end
		...
		if not self:Validate(self.e) then --If it doesn't have an enemy.
			...
			if !uvenemyescaping and self:StraightToTarget(self.e) then
				...
				self:ObstaclesNearbySide() or self.v.rhino then
					if (!self.driveinfront or self:ObstaclesNearbySide()) and !self.v.rhino then
			...
			elseif self.v.rhino then --Getting unstuck
				...
				if edist:Dot(forward) > 0 and eeeevectdot < 0 and self.e:GetVelocity():LengthSqr() > 100000 and (self:StraightToTarget(self.e) or !self.aggressive) then 
				...
				if self.v:GetVelocity():LengthSqr() > self.e:GetVelocity():LengthSqr() and edist:Dot(forward) > 0 and edist:Dot(eforward) > 0 and eevectdot > 0 and eeevectdot > 0 and edist:Length2DSqr() < 250000 and self.e:GetVelocity():LengthSqr() > 250000 and not uvcalm then
					if self.aggressive and !self.formationpoint and eright.z > -0.5 and eright.z < 0.5 then throttle = 2 end
				...
				--if self.v:GetVelocity():LengthSqr() < 100000 and self.e:GetVelocity():LengthSqr() > 100000 and evectdot < 0 then throttle = 0 end --Create roadblock	
				...
				--Ramming
				if edist:Dot(forward) > 0 and eeevectdot < 0 and self.e:GetVelocity():LengthSqr() > 100000 and (self:StraightToTarget(self.e) or !self.aggressive) then 
					if !Relentless:GetBool() or (self.v:GetVelocity():LengthSqr()+self.e:GetVelocity():LengthSqr()) > eedist:Length2DSqr() then
						if self.v:GetVelocity():LengthSqr() > 123904 then 
							throttle = 0 
						end
						if dist:Dot(eforward) < 0 then
							if eright.z < 0 then 
			...
			--Deploying spike strips/setting up tactical roadblocks
			if uvcalm or uvenemyescaping or !self.aggressive then
				self.rdeploying = CurTime()
				self.deploying = CurTime()
			end
			if not (dist:Dot(forward) < 0 and eeevectdot < 0 and dist:Length2DSqr() < 25000000 and dist:Length2DSqr() > 250000) then
			    self.deploying = CurTime()
			end
			if not (eedist:Length2DSqr() > 25000000 and eedist:Length2DSqr() < 100000000 and eeevectdot < 0 and self.e:GetVelocity():LengthSqr() > 250000) then
				self.rdeploying = CurTime()
			end
			if self.e:GetVelocity():LengthSqr() < 100000 then self.deploying = CurTime() end
			if self.v:GetNoDraw(true) and self.v:GetCollisionGroup(20) and dist:Length2DSqr() < 25000000 then
				self.deploying = CurTime()
			end
			...
			if !uvenemyescaping then uverespawn = self.e:GetPos() end
			self:SetELSSiren(true)
			if Chatter:GetBool() and IsValid(self.v) and not uvcalm and !uvenemyescaping and self.driveinfront then
	...
	function ENT:Initialize()
		...
		self:SetHealth(-1)
		self.bountytimer = CurTime()
		self.callsign = "Special "..self:EntIndex()
		self.moving = CurTime()
		self.deploying = CurTime()
		self.rdeploying = CurTime()
		self.ks = CurTime()
			...
			if IsValid(self.v) then 
				if self.v:GetClass() == "prop_vehicle_jeep" then
					self.v:AddCallback("PhysicsCollide", function(ent, coldata)
						...
						if object.UVWanted then
							if car.rhino and !car.rhinohit then
								car.rhinohit = true
							end
						end
						if dmg < 100 or self.toofar or !CanWreck:GetBool() then
							return
						end
		...
		if self.vehicle then
			...
			if v.UnitVehicle then return end --If it's already a Unit Vehicle, don't pick it up again.
		...
		if !uvtargeting then self.v:EmitSound( "plats/tram_hit1.wav" ) end

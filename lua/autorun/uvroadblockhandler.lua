...
if SERVER then
    ...
    util.AddNetworkString("UVAddRoadblock")
    util.AddNetworkString("UVTriggerRoadblock")
	    	...
	    	timer.Simple(10, function()
	    		...
	    		timer.Create("uvroadblockmarkedfordeletion"..Index, 1, 0, function() 
	    			if IsValid(gib) then
	    				...
	    				if distance > 100000000 and IsValid(gib) or !uvtargeting then
	    					gib:Remove()
        ...
        for k, ent in pairs( entities ) do
			...
			if !manual then
            	timer.Simple(10, function()
					...
					timer.Create("uvroadblockmarkedfordeletion"..Index, 1, 0, function() 
						if !IsValid(gib) or !uvtargeting then
							if IsValid(ent) then
								ent:Remove()
	...
	function UVAutoLoadRoadblock()
        ...
        for k, jsonfile in pairs(availableroadblocks) do
        	...
        	if (distance:LengthSqr() < 25000000 or distance:LengthSqr() > 100000000 or evectdot > 0) then
        	    continue
        	end
